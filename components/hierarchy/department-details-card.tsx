...
export const DepartmentDetailsCard = memo(function DepartmentDetailsCard({ 
  ...
  const sortedUsers = useMemo(() => 
    [...data.users].sort((a, b) => {
      ...
      if (a.reportStatus.taskCompletionRate !== b.reportStatus.taskCompletionRate) {
        return b.reportStatus.taskCompletionRate - a.reportStatus.taskCompletionRate
      }
      return `${a.firstName} ${a.lastName}`.localeCompare(`${b.firstName} ${b.lastName}`)
    }), [data.users]
  )

  const getStatusColor = useMemo(() => (user: any) => {
  ...
  const getStatusBadge = useMemo(() => (user: any) => {
    if (!user.reportStatus.hasReport) {
      return { label: 'Chưa nộp', variant: 'secondary' as const, color: 'bg-gray-500' }
    }
    if (user.reportStatus.isCompleted) {
      return { label: 'Hoàn thành', variant: 'default' as const, color: 'bg-green-500' }
    }
    if (user.reportStatus.taskCompletionRate >= 70) {
      return { label: 'Đang làm', variant: 'outline' as const, color: 'bg-yellow-500' }
    }
    return { label: 'Cần theo dõi', variant: 'destructive' as const, color: 'bg-red-500' }
  }, [])
    ...
    <div className="space-y-6">
      ...
      <Card>
            ...
            <div className="flex items-center gap-4">
              <div className="text-center">
                <SimplePieChart
                  ...
                  incomplete={summaryStats.totalUsers - summaryStats.usersWithReports}
                  size={100}
                  strokeWidth={10}
                  showLabel
                />
        ...
        {data.department.description && (
          <CardContent>
            <p className="text-muted-foreground">{data.department.description}</p>
          </CardContent>
        )}
      </Card>
                      ...
                      <div className="block lg:hidden p-4">
                        ...
                        {user.reportStatus.hasReport && (
                          <div className="grid grid-cols-2 gap-2 text-center text-xs mb-3">
                            <div>
                              <span className="text-muted-foreground">Tổng CV:</span>
                          ...
                          <div className="text-right">
                            {user.reportStatus.hasReport && (
                              <div className="mb-3">
                                ...
                                <div className={`text-2xl font-bold ${getStatusColor(user)}`}>
                        ...
                        {user.reportStatus.incompleteReasons.length > 0 && (
                          <div className="mt-4 p-3 bg-orange-50 rounded-lg border border-orange-100">
                            ...
                            <div className="space-y-1 max-h-24 overflow-y-auto">
                              {user.reportStatus.incompleteReasons.map((item, idx) => (
                                <div key={idx} className="text-xs">
                                  <span className="font-medium text-orange-800">{item.taskName}:</span>
